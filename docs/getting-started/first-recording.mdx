...
<Steps>
  ...
  <Step title="Press Record">
    ...
    Steno starts capturing audio immediately. You will see a live timer in the top bar, and with the default Parakeet engine a live transcript appears while you record.
  </Step>
...
Transcription speed depends on your hardware and which engine you use. It runs entirely on-device and is fast on Apple Silicon; the default Parakeet engine also shows a live transcript as you record. Intel Macs and the optional Whisper engine run more slowly. You can switch engines in **Settings → Transcription**.
</Accordion>

<Accordion title="The summary is not very useful -- what can I do?">
Try a larger summarization model. In **Settings → Models**, switch from the default `gemma4:e2b-it-qat` to `qwen3.5:9b` or `gpt-oss:20b`. Larger models produce better-structured notes but take longer to run.
</Accordion>
