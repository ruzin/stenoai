...
2. **Transcribe** -- runs a local speech-to-text engine to produce a verbatim transcript with speaker labels. Steno ships two engines: Parakeet (the default, which also shows a live transcript while you record) and Whisper (an optional alternate you can switch to in Settings)
...
Every step runs offline. Steno does not communicate with any external server unless you explicitly configure a cloud model (OpenAI, Anthropic, AWS Bedrock, or a custom OpenAI-compatible endpoint) as an optional alternative to the local model.
