...
Larger models produce better answers. In **Settings → Models → Summarization**, switch to `qwen3.5:9b` or `gemma4:12b-it-qat`. Also check that the full transcript was saved -- if transcription failed or was skipped, the model only has the summary to work from.
</Accordion>
