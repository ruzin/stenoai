...
Steno records the microphone and system audio as two separate channels. The default Parakeet engine uses this stereo separation to diarize the transcript -- lines from the microphone channel are labeled `[You]` and lines from the system audio channel are labeled `[Others]`.

## Requirements

Speaker labels require **system audio capture** with the default **Parakeet** engine. Microphone-only recordings produce a single-channel transcript without speaker differentiation. Recordings transcribed with the Whisper engine are mono and do not carry speaker labels.
