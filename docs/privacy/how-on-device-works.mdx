...
Steno processes your recordings using two AI models that run locally on your hardware. After the initial model download, the app makes no network requests related to your recordings. Your audio, transcripts, and summaries never leave your Mac.
...
Audio file (local)
  ...
  → transcript.md (saved locally)
  → Ollama (on-device LLM)
  → summary.md (saved locally)
```

No step in this pipeline contacts an external server. The models are downloaded once during setup and then run offline indefinitely.
...
Steno bundles [Ollama](https://ollama.com), which manages and runs small language models locally. When Steno starts, it launches a local Ollama server that listens only on `127.0.0.1` (loopback). All summarization and query requests go to `http://localhost:11434` -- they never leave your machine.
...
No. Steno does not include any analytics, crash reporting, or telemetry. The only outbound requests are the update check on launch and model downloads during setup.
</Accordion>
