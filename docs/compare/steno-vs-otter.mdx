...
| **Supported languages** | European on the default engine; 99 via Whisper | Primarily English |
...
Otter is primarily optimized for English. Steno's default Parakeet engine covers major European languages; switching to the Whisper engine adds automatic detection across 99 languages.
...
Both use strong transcription engines and perform similarly for clear, English-language audio. Steno's main advantage is that it runs on-device without sending audio anywhere; its default Parakeet engine covers major European languages, and the optional Whisper engine adds language breadth (99 languages vs Otter's primarily English focus).
</Accordion>
