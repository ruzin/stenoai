...
// --- Soft-delete / trash support (#234) -----------------------------------
// Note deletion moves files into <userData>/.trash/<id>/ instead of unlinking,
// so an Undo toast can restore them for a few seconds. Leftover trash is
// hard-deleted at the next launch (the undo window ends at quit).

// Root of the trash tree. A sibling of output/recordings/transcripts under the
// data dir, so the Python `list_meetings` glob (which scans output/) never sees
// trashed notes. It sits INSIDE getUserDataDir(), an allowed base dir, so trash
// paths validate for both the move-in and move-out directions with no extra
// allowed dir.
function getTrashRoot() {
  return path.join(getUserDataDir(), '.trash');
}

// Move a file, preferring an atomic same-filesystem rename. A custom storage
// path may live on a different volume than getUserDataDir(), where rename fails
...
// A trashId is generated by us (Date.now()-<hex>), but the restore/purge
// handlers receive it back from the untrusted renderer, so reject anything with
// a path separator or parent-dir token before it's ever joined onto a path.
function isSafeTrashId(trashId) {
  return (
...
// The trash ROOT must never be a symlink. If `.trash` were symlinked to an
// allowed dir (e.g. <userData> itself), a renderer could pass a real basename
// there — trashId "output" or "config.json" — and have restore/purge rmSync an
// arbitrary target: the child path would resolve inside an allowed base and
// slip past validateSafeFilePath. The startup sweep already refuses a symlinked
// root (isDirectory() is false for a symlink under lstat); restore + purge
// mirror that guard here. A MISSING root is fine (nothing to traverse — the
// per-entry existsSync handles it), so only an existing-but-not-real-directory
// root (symlink or file) is rejected.
function isTrashRootSafe() {
  try {
    return fs.lstatSync(getTrashRoot()).isDirectory();
  } catch (_) {
    return true; // No trash root yet — no escape vector.
  }
}
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    ...
    // Hard-delete leftover soft-deleted notes (#234): any trash still on disk is
    // from a prior session whose Undo window ended at quit. Best-effort — a
    // failure here must never block launch. Cross-platform (trash works on both
    // macOS and Windows); getUserDataDir() honors STENOAI_USER_DATA_DIR so
    // tests/e2e stay isolated.
    try {
      purgeAllTrashOnLaunch();
    } catch (e) {
      console.warn('trash purge on launch failed (non-fatal):', e?.message);
    }
...
// Soft-delete a note: MOVE its files into <userData>/.trash/<trashId>/ and
// return the trashId so the renderer can offer Undo (#234). Nothing is unlinked
// here — an expired/dismissed undo calls purge-trashed-meeting; Undo calls
// restore-meeting. Leftover trash is hard-deleted at the next launch.
ipcMain.handle('delete-meeting', async (event, meetingData) => {
  try {
    ...
    const absolutePaths = [];
    if (summaryFile) absolutePaths.push(toAbs(summaryFile));
    if (transcriptFile) absolutePaths.push(toAbs(transcriptFile));
    if (audioFile) absolutePaths.push(toAbs(audioFile));
    if (summaryFile && sessionName) {
      const outputDir = path.dirname(toAbs(summaryFile));
      const safeName = sessionName.replace(/[^a-zA-Z0-9_-]/g, '_');
      absolutePaths.push(path.join(outputDir, `${safeName}_notes.txt`));
    }
    // Reports sidecar (src/report_store.sidecar_path): the summary file's
    // `_summary.md`/`_summary.json` suffix becomes `_reports.json`. Previously
    // orphaned on delete — include it so restore is faithful and the orphan is
    // fixed. Only moved below if it actually exists.
    if (summaryFile) {
      const abs = toAbs(summaryFile);
      const dir = path.dirname(abs);
      const base = path.basename(abs);
      let sidecarBase = null;
      for (const suf of ['_summary.md', '_summary.json']) {
        if (base.endsWith(suf)) {
          sidecarBase = base.slice(0, -suf.length) + '_reports.json';
          break;
        }
      }
      if (!sidecarBase) {
        const ext = path.extname(base);
        sidecarBase = base.slice(0, base.length - ext.length) + '_reports.json';
      }
    ...
    if (summaryFile) {
      ...
      if (stem) {
        ...
        // The recording keeps its ORIGINAL extension (Codex C5): imports stay
        // .m4a/.mp3/…, system-audio is .webm, only native recordings are .wav —
        // so hard-coding <stem>.wav orphans the rest. Include every recordings/
        // entry whose stem matches (<stem>.<ext>, i.e. path.parse(name).name ===
        // stem). These candidates still pass the existsSync + isFile +
        // isAllowedMeetingSource gates below, so there's no security regression.
        ...
        try {
          for (const name of fs.readdirSync(recordingsDir)) {
            if (path.parse(name).name === stem) {
              absolutePaths.push(path.join(recordingsDir, name));
            }
          }
        } catch (_) {
    ...
    // Security pre-pass: validate EVERY candidate before moving anything, so a
    // rejected path (e.g. a symlink escaping the data dir) blocks the whole
    // delete with nothing half-moved. Preserves the delete-meeting security test.
    ...
    return {
      success: true,
      trashId,
      message: `Moved meeting to trash (${movedFiles.length} files)`
    };
  } catch (error) {
...
// Undo a soft-delete: move every file recorded in the trash manifest back to
// its original path, then remove the trash dir (#234).
ipcMain.handle('restore-meeting', async (event, trashId) => {
  try {
    if (!isSafeTrashId(trashId)) {
      return { success: false, error: 'Invalid trash id' };
    }
    // Reject a symlinked/non-directory trash root before joining trashId onto it
    // (shared guard — see isTrashRootSafe).
    if (!isTrashRootSafe()) {
      return { success: false, error: 'Invalid trash root' };
    }
    const allowedBaseDirs = getAllowedBaseDirs();
    const trashDir = path.join(getTrashRoot(), trashId);
    const manifestPath = path.join(trashDir, 'manifest.json');
    if (!fs.existsSync(manifestPath)) {
    ...
    if (!validateSafeFilePath(trashDir, allowedBaseDirs)) {
      return { success: false, error: 'Invalid trash path' };
    }
    ...
    // A missing/empty/non-array files list is a MALFORMED manifest (Codex M2),
    // NOT a successful zero-file restore. Fail WITHOUT removing the dir so the
    // trashed files (if any) are never silently orphaned.
    if (!Array.isArray(manifest.files) || manifest.files.length === 0) {
      return {
        success: false,
        error: 'Empty or malformed trash manifest',
    ...
    for (const entry of manifest.files) {
      ...
      // basename-equality check already forbids any path separator, and the
      // realpath-in-trashDir check below forbids traversal, so a substring `..`
      // is harmless — reject ONLY the exact `.`/`..` names (Codex M1), keeping
      // legit filenames like `0__Q1..final.m4a`.
      if (
        ...
        return {
          success: false,
          error: 'Invalid stored name in trash manifest',
          meeting: manifest.meeting,
        };
      }
      ...
      if (realStored !== realTrashDir && !realStored.startsWith(realTrashDir + path.sep)) {
        ...
        return {
          success: false,
          error: 'Trash entry resolves outside trash dir',
          meeting: manifest.meeting,
        };
      }
    ...
    for (const { stored, dest } of plan) {
      try {
        ...
        console.error(`Restore: I/O fault moving ${stored} -> ${dest}:`, err.message);
        return {
          success: false,
          error: `Restore failed mid-move: ${err.message}`,
          meeting: manifest.meeting,
        };
      }
    }
...
// Hard-delete a trashed note (undo window expired or the toast was dismissed).
// Idempotent: a missing trash dir is a success (#234).
ipcMain.handle('purge-trashed-meeting', async (event, trashId) => {
  try {
    if (!isSafeTrashId(trashId)) {
      return { success: false, error: 'Invalid trash id' };
    }
    // Reject a symlinked/non-directory trash root before joining trashId onto it
    // (shared guard — see isTrashRootSafe). Without this, a `.trash` symlinked to
    // an allowed dir would let trashId "config.json" resolve to a real file that
    // passes validateSafeFilePath and gets rmSync'd.
    if (!isTrashRootSafe()) {
      return { success: false, error: 'Invalid trash root' };
    }
    const allowedBaseDirs = getAllowedBaseDirs();
    const trashDir = path.join(getTrashRoot(), trashId);
    // Idempotent: a missing trash dir is a success (nothing to hard-delete).
    ...
    // Only hard-delete an ACCOUNTABLE trash entry — a dir carrying a
    // manifest.json (mirrors the startup sweep). A dir without one is not a real
    // trash entry we created, so leave it untouched and report idempotent
    // success rather than rmSync an unaccountable directory.
    if (!fs.existsSync(path.join(trashDir, 'manifest.json'))) {
      console.warn(`Purge: skipping unmanifested trash entry (preserved): ${trashDir}`);
      return { success: true };
    }
    fs.rmSync(trashDir, { recursive: true, force: true });
    return { success: true };
  } catch (error) {
    console.error('Purge trashed meeting error:', error);
    return { success: false, error: error.message };
  }
});
