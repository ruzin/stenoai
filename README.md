...
- **Global record shortcut** — Start or stop recording from anywhere with `⌘⇧R` (`Ctrl+Shift+R` on Windows). Toggle it off in Settings if it clashes with another app, and power users can bind any key of their own via the `stenoai://record/start` / `record/stop` deep links (macOS Shortcuts, AutoHotkey).
